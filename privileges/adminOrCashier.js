...
const ensureAdminOrCashier = async (req, res, next) => {
    try {
        ...
        if (rows.length > 0 && rows[0].name) {
            ...
            if (rows[0].name === 'admin' || rows[0].name === 'cashier') {
                return next(); // Allow access if the user is an admin or cashier
            } else {
                return res.status(403).send('Access denied: Admins or Cashiers only.');
            }
        } else {
